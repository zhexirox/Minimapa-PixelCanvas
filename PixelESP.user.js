...
window.addEventListener('load', function() {
  ...
  document.getElementById("hide-map").onclick = function(){
    console.log("[MiniMap] This should do something, but it doesn't");
    toggle_show = false;
...
function updateloop(){
  console.log("[MiniMap] Updating Template List");
  // Get JSON of available templates
  var xmlhttp = new XMLHttpRequest();
  //var url = "https://api.myjson.com/bins/1ha4zt"; 
  var url = "https://api.myjson.com/bins/wermb";
...
function loadTemplates(){
  ...
  for(i = 0; i < keys.length; i++) {
    ...
    needed_templates.push(template_list[template]);
  }
...
function loadImage(imagename){
  console.log("[MiniMap] Load image " + imagename.filename);
  image_list[imagename.filename] = new Image();
  if(cachebreaker != null)
    image_list[imagename.filename].src = "http://pixelesp.likehosting.net/_templates/" + imagename.filename + "&cachebreaker=" + cachebreaker;
  else
    image_list[imagename.filename].src = "http://pixelesp.likehosting.net/_templates/" + imagename.filename;
  image_list[imagename.filename].onload = function() {
    counter += 1;
    //if last needed image loaded, start drawing
    if (counter == needed_templates.length)
      drawTemplates();
...
function drawTemplates(){
  ...
  for(i = 0; i < needed_templates.length; i++){
    ...
    var newwidth = zoomlevel * image_list[template.filename].width;
    var newheight = zoomlevel * image_list[template.filename].height;
    var img = image_list[template.filename];
